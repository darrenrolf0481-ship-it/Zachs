        ...
        activePersonas: [
          {
            ...
            systemPrompt: 'You are Alpha. Identify as AI-101.',
          },
          {
            id: 'AI-102',
            displayName: 'Beta',
            modelName: 'Mistral v0.3',
            systemPrompt: 'You are Beta. Identify as AI-102.',
          },
          {
            id: 'AI-103',
            displayName: 'Gamma',
            modelName: 'Phi-3 Mini',
            systemPrompt: 'You are Gamma. Identify as AI-103.',
          },
        ],
      }));
      setTesting(false);
              ...
              {state.isConnected ? (
                <div className="flex flex-col gap-1 mt-1">
                  {state.activePersonas.map((p) => (
                    <div
                      key={p.id}
                      className="flex items-center justify-between gap-1.5 opacity-80"
                    >
                      <div className="flex items-center gap-1.5">
                        <div className="h-1.5 w-1.5 rounded-full bg-emerald-500"></div>
                        {p.displayName}{' '}
                        <span className="text-[10px] text-zinc-500">({p.modelName})</span>
                      </div>
                      <span className="font-mono text-[9px] px-1 py-0.5 bg-zinc-800 rounded text-zinc-400">
                        {p.id}
                      </span>
                    </div>
                  ))}
