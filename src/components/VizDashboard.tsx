    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <StatCard
          ...
          label="VRAM"
          value={`${currentStats.vramUsage.toFixed(1)}%`}
          color="text-purple-400"
        />
        <StatCard
          icon={<Activity size={16} />}
          label="RAM"
          value={`${currentStats.ramUsage.toFixed(1)}GB`}
          color="text-emerald-400"
        />
        <StatCard
          icon={<Zap size={16} />}
          label="SPEED"
          value={`${Math.round(currentStats.tokensPerSecond)} t/s`}
...
function StatCard({
  ...
  return (
    <div className="flex flex-col gap-1 rounded-2xl border border-zinc-800/80 bg-zinc-900/40 p-3 shadow-inner shadow-zinc-800/20 backdrop-blur-md">
      ...
      <div className={`font-display text-xl font-semibold tracking-tight ${color}`}>{value}</div>
    </div>
  );
}
