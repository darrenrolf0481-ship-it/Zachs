...
export function ChatInterface({ isConnected, activePersonas }: { isConnected: boolean, activePersonas: Persona[] }) {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    const personasToQuery = selectedModel === "all" ? activePersonas : activePersonas.filter(p => p.id === selectedModel);
    personasToQuery.forEach((persona, idx) => {
      ...
      }, 800 + (idx * 600)); // Stagger the responses slightly
    });
  };

  const getModelColor = (modelName?: string) => {
    if (!modelName) return "border-zinc-700/50 bg-zinc-800 text-zinc-200";
    if (modelName.toLowerCase().includes("llama")) return "border-cyan-500/30 bg-cyan-950/20 text-cyan-50";
    if (modelName.toLowerCase().includes("mistral")) return "border-purple-500/30 bg-purple-950/20 text-purple-50";
    if (modelName.toLowerCase().includes("phi")) return "border-emerald-500/30 bg-emerald-950/20 text-emerald-50";
    return "border-zinc-700/50 bg-zinc-800 text-zinc-200";
  };

  const getModelMetadata = (modelName: string) => {
    switch (modelName) {
      case "Llama 3 (8B)": return { params: "8.0B", quant: "Q4_K_M", ctx: "8k", vram: "5.8 GB", vramPercent: 48 };
      case "Mistral v0.3": return { params: "7.3B", quant: "Q5_K_M", ctx: "32k", vram: "6.2 GB", vramPercent: 52 };
      case "Phi-3 Mini": return { params: "3.8B", quant: "Q4_K_M", ctx: "4k", vram: "2.7 GB", vramPercent: 22 };
      default: return { params: "?", quant: "?", ctx: "?", vram: "?", vramPercent: 0 };
    }
  };
            ...
            {messages.map((msg) => (
              <motion.div
                ...
                className={cn(
                  ...
                  msg.role === "system" ? "self-center border border-zinc-800 bg-zinc-900/50 text-center text-xs text-zinc-500" :
