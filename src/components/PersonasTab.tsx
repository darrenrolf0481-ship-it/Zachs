  ...
  if (!state.isConnected) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="mb-2 text-xl font-display font-semibold text-zinc-300">
          Identity Drift Protection
        </h2>
        <p className="text-sm text-zinc-500">
          Connect to a server first to configure mod files and personas.
        </p>
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col gap-6 p-6 overflow-y-auto custom-scrollbar pb-24">
      <div className="flex flex-col gap-2 relative">
        ...
        <p className="text-sm text-zinc-400">
          Set up Identity Drift Protection. Ensure each model recognizes its ID # and has the
          correct Mod File (System Prompt).
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {state.activePersonas.map((persona) => (
          <div
            key={persona.id}
            className="flex flex-col gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 shadow-xl backdrop-blur-md"
          >
